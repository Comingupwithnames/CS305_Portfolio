 ...
 ****************************************************
 */
void add_item(llnode** cart_ptr, retail_item* it) {
  llnode* crt = *cart_ptr;
  // currently nothing in list, so create a new node
  // and assign *cart_ptr to the new node
  // so *cart_ptr is no longer NULL
  if(crt == NULL) {
    crt = (llnode *) malloc(sizeof(llnode));
    crt->item = it;
    crt->next = NULL;
    *cart_ptr = crt;
    return;
